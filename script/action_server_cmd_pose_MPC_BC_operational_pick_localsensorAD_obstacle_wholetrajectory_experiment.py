...
#        builder_wholebodyMPC_planner.add_equality_constraint('final_dr_middle', dr_middle_var_MPC[:,-1], rhs=np.zeros(3))
        ...
        lower_velocity = np.array([-0.9, -0.9, -0.7]); upper_velocity = np.array([0.9, 0.9, 0.7]);
...
#        builder_wholebodyMPC_planner.add_equality_constraint('final_ddr_middle', ddr_middle_var_MPC[:,-1], rhs=np.zeros(3))
        ...
        lower_acceleration = np.array([-1, -1, -0.7]); upper_acceleration = np.array([1, 1, 0.7]);
...
#            F_ext_Left_var_MPC[:, i] = F_ext_Left_actual + inertia_Left @ ddDelta_p_Left_var_MPC[:, i] + self.K_Left @ Delta_p_Left_var_MPC[:, i] + self.D_Left @ dDelta_p_Left_var_MPC[:, i]
            ...
            builder_wholebodyMPC.add_cost_term('Right_arm position AD' + str(i), 6*optas.sumsqr(self.pos_fnc_Right(q_var_MPC[:, i])-pos_R_reasonal[:, i] - self.rotation_fnc_Right(q_var_MPC[:, i]) @ Delta_p_Right_var_MPC[:, i]))
            builder_wholebodyMPC.add_cost_term('Left_arm position AD' + str(i),  6*optas.sumsqr(self.pos_fnc_Left(q_var_MPC[:, i])-pos_L_reasonal[:, i]  - self.rotation_fnc_Left(q_var_MPC[:, i]) @ Delta_p_Left_var_MPC[:, i]))
