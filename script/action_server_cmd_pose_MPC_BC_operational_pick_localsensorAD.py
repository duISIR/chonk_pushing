...
class CmdPoseActionServer(object):
    ...
    def __init__(self, name):
        ...
        self.F_ext_local_Right = np.zeros(6);  self.self.F_ext_local_Left = np.zeros(6);
...
#        Global_X_ee_left[3:6, 3:6] = self.rotation_fnc_Left(init_position_MPC)
        for i in range(self.T_MPC):
            ...
            builder_wholebodyMPC.add_cost_term('Right_arm position AD' + str(i), 3*optas.sumsqr(self.pos_fnc_Right(q_var_MPC[:, i])-pos_R[:, i] - init_Delta_position_Right - self.rotation_fnc_Right(q_var_MPC[:, i]) @ Delta_p_Right_var_MPC[:, i]))
            builder_wholebodyMPC.add_cost_term('Left_arm position AD' + str(i),  3*optas.sumsqr(self.pos_fnc_Left(q_var_MPC[:, i])-pos_L[:, i]  - init_Delta_position_Left  - self.rotation_fnc_Left(q_var_MPC[:, i]) @ Delta_p_Left_var_MPC[:, i]))
            #####################################################################################
...
#                                                           'F_ext_Right_actual': self.F_ext_global_Right[3:6], 'F_ext_actual_local': self.F_ext_global_Left[3:6],
