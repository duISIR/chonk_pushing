...
class CmdForceActionServer(object):
...
#        self.dt_MPC_planner = 0.1 # time step
        self.T_MPC_planner = 11 # T is number of time steps
...
#        builder_wholebodyMPC_planner.add_equality_constraint('Final_middle', r_middle_var_MPC[:, self.T_MPC_planner-1], rhs=0.5*(pos_R+pos_L))
        ...
        for i in range(self.T_MPC_planner):
            obstacle_pos = np.asarray([[4.45], [1.15]])
            obstacle_radius = 0.9
...
#        dr_middle_var_MPC = optas.casadi.SX(np.zeros((3, self.T_MPC_planner)))
        ...
        w_dr = 0.01/float(self.T_MPC_planner)
        for i in range(self.T_MPC_planner):
...
#        ori_L = builder_wholebodyMPC.add_parameter('ori_L', 4, self.T_MPC)
        ...
        self.pos_R_reference = np.zeros((3, self.T_MPC))
        self.pos_L_reference = np.zeros((3, self.T_MPC))
        self.ori_R_reference = np.zeros((4, self.T_MPC))
        self.ori_L_reference = np.zeros((4, self.T_MPC))
...
#        Global_X_ee_Left = optas.casadi.SX(np.zeros((6, 6)))
        ...
        #####################################################################################
        r_middle_var_MPC = optas.casadi.SX(np.zeros((2, self.T_MPC)))
        donkey_transform = self.wholebodyMPC.get_global_link_transform_function(link=self._link_donkey)
        donkey_pos1 = optas.casadi.SX(np.zeros((2, self.T_MPC)))
        donkey_pos2 = optas.casadi.SX(np.zeros((2, self.T_MPC)))
...
#            builder_wholebodyMPC.add_cost_term('Left_arm position AD' + str(i),  optas.sumsqr(self.pos_fnc_Left(q_var_MPC[:, i])-pos_L_reasonal[:, i]  - init_Delta_position_Left  - self.rotation_fnc_Left(init_position_MPC) @ Delta_p_Left_var_MPC[:, i]))
        ...
        for i in range(self.T_MPC):
            if(i>0):
                r_middle_var_MPC[:, i] = 0.5*(self.pos_fnc_Left(q_var_MPC[:, i])[0:2] + self.pos_fnc_Left(q_var_MPC[:, i])[0:2])
                obstacle_pos = np.asarray([[4.45], [1.15]])
                obstacle_radius = 0.9
                donkey_pos1[:, i] = (donkey_transform(q_var_MPC[:, i]) @ np.array([0.3+0.48, 0, 0, 1]))[0:2]
                donkey_pos2[:, i] = (donkey_transform(q_var_MPC[:, i]) @ np.array([0.3-0.48, 0, 0, 1]))[0:2]
...
#        builder_wholebodyMPC.add_equality_constraint('init_Delta_position_Left_constraint_y',  P_Left[:, 0],  rhs = np.zeros(3) )
        ...
        for i in range(self.T_MPC):
            ...
            builder_wholebodyMPC.add_cost_term('minimize_velocity_base' + str(i), w_dq * optas.sumsqr(dq_var_MPC[0:3, i]))
            builder_wholebodyMPC.add_cost_term('minimize_velocity_joint' + str(i), 0.003 * optas.sumsqr(dq_var_MPC[3:, i]))
        ...
        for i in range(self.T_MPC):
            ...
            builder_wholebodyMPC.add_cost_term('minimize_acceleration_base' + str(i), w_ddq * optas.sumsqr(ddq_var_MPC[0:3, i]))
            builder_wholebodyMPC.add_cost_term('minimize_acceleration_joint' + str(i), 0.005 * optas.sumsqr(ddq_var_MPC[3:, i]))
...
#        self._joint_sub_base = rospy.Subscriber("/chonk/base_pose_ground_truth", Odometry, self.read_base_states_cb)
        self._motion_reference_right_sub = rospy.Subscriber("/chonk/motion_reference_right", Float64MultiArray, self.read_motion_reference_right_cb)
        self._motion_reference_left_sub = rospy.Subscriber("/chonk/motion_reference_left", Float64MultiArray, self.read_motion_reference_left_cb)

        self._idx_sub = rospy.Subscriber("/chonk/idx", Int32, self.read_idx_cb)
...
#                self.dq_curr = np.concatenate((self.dq_curr_base, self.dq_curr_joint), axis=None)
                ...
                self.solver_wholebodyMPC.reset_parameters({'pos_R_reasonal': self.pos_R_reference, 'pos_L_reasonal': self.pos_L_reference,
                                                           'ori_R_reasonal': self.ori_R_reference, 'ori_L_reasonal': self.ori_L_reference,
                                                           't': self.timebyT,
...
#        self.dq_curr_base = [float(msg.twist.twist.linear.x), float(msg.twist.twist.linear.y), float(msg.twist.twist.angular.z)]
    ...
    def read_motion_reference_right_cb(self, msg):
        self.pos_R_reference[0, :] = np.array(msg.data[0*self.T_MPC:1*self.T_MPC])
        self.pos_R_reference[1, :] = np.array(msg.data[1*self.T_MPC:2*self.T_MPC])
        self.pos_R_reference[2, :] = np.array(msg.data[2*self.T_MPC:3*self.T_MPC])

        self.ori_R_reference[0, :] = np.array(msg.data[3*self.T_MPC:4*self.T_MPC])
        self.ori_R_reference[1, :] = np.array(msg.data[4*self.T_MPC:5*self.T_MPC])
        self.ori_R_reference[2, :] = np.array(msg.data[5*self.T_MPC:6*self.T_MPC])
        self.ori_R_reference[3, :] = np.array(msg.data[6*self.T_MPC:7*self.T_MPC])


    def read_motion_reference_left_cb(self, msg):
        self.pos_L_reference[0, :] = np.array(msg.data[0*self.T_MPC:1*self.T_MPC])
        self.pos_L_reference[1, :] = np.array(msg.data[1*self.T_MPC:2*self.T_MPC])
        self.pos_L_reference[2, :] = np.array(msg.data[2*self.T_MPC:3*self.T_MPC])

        self.ori_L_reference[0, :] = np.array(msg.data[3*self.T_MPC:4*self.T_MPC])
        self.ori_L_reference[1, :] = np.array(msg.data[4*self.T_MPC:5*self.T_MPC])
        self.ori_L_reference[2, :] = np.array(msg.data[5*self.T_MPC:6*self.T_MPC])
        self.ori_L_reference[3, :] = np.array(msg.data[6*self.T_MPC:7*self.T_MPC])

    def read_idx_cb(self, msg):
        self._idx_planner = msg.data
...
if __name__=="__main__":
    # Initialize node
    rospy.init_node("cmd_force_server", anonymous=True)
    # Initialize node class
    cmd_force_server = CmdForceActionServer(rospy.get_name())
    # executing node
    rospy.spin()
