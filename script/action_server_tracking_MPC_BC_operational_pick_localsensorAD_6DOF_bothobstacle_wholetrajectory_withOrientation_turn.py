...
from pushing_msgs.msg import CmdChonkForceAction, CmdChonkForceFeedback, CmdChonkForceResult
...
#        )
        ...
        self._motion_reference_sub = rospy.Subscriber("/chonk/motion_reference", Float64MultiArray, self.read_motion_reference_cb)
...
#        )
        ...
        self._feedback = CmdChonkForceFeedback()
        self._result = CmdChonkForceResult()
    ...
    def goal_cb(self):
        ...
        # desired end-effector force and torque
...
#        self.ft_left = -np.asarray([msg.wrench.torque.x, msg.wrench.torque.y, msg.wrench.torque.z, msg.wrench.force.x, msg.wrench.force.y, msg.wrench.force.z])
    ...
    def read_motion_reference_cb(self, msg):
        ...
        self.pos_L_reference[0, :] = np.array(msg.data[3*self.T_MPC:4*self.T_MPC])
        self.pos_L_reference[1, :] = np.array(msg.data[4*self.T_MPC:5*self.T_MPC])
        self.pos_L_reference[2, :] = np.array(msg.data[5*self.T_MPC:6*self.T_MPC])

        self.ori_R_reference[0, :] = np.array(msg.data[6*self.T_MPC:7*self.T_MPC])
        self.ori_R_reference[1, :] = np.array(msg.data[7*self.T_MPC:8*self.T_MPC])
        self.ori_R_reference[2, :] = np.array(msg.data[8*self.T_MPC:9*self.T_MPC])
        self.ori_R_reference[3, :] = np.array(msg.data[9*self.T_MPC:10*self.T_MPC])

        self.ori_L_reference[0, :] = np.array(msg.data[10*self.T_MPC:11*self.T_MPC])
        self.ori_L_reference[1, :] = np.array(msg.data[11*self.T_MPC:12*self.T_MPC])
        self.ori_L_reference[2, :] = np.array(msg.data[12*self.T_MPC:13*self.T_MPC])
        self.ori_L_reference[3, :] = np.array(msg.data[13*self.T_MPC:14*self.T_MPC])
...
if __name__=="__main__":
    # Initialize node
    rospy.init_node("tracking_server_MPC_BC_operational", anonymous=True)
